...
export const ChangelogLayout = (props: ChangelogLayoutProps) => {
  ...
  React.useLayoutEffect(() => {
    ...
    const timeout = setTimeout(() => {
      ...
      const articleMonth = dayjs(props.publishedAt).format("MM");
  ...
  const Changelog = () => (
    <>
      ...
      <Timeline
        ...
        date={dayjs(props.publishedAt).format("MMM DD YYYY")}
        className={`timeline-month-${dayjs(props.publishedAt).format("MM")}`}
      >
        <Box
        ...
        >
          ...
          <VStack align="start" spacing={[4, 4, 6]}>
            ...
            <motion.div
              layoutId={shouldAnimateFromPreviousPage ? `${props.slug}` : ``}
              initial={{
            ...
            >
              {isInBlogPage ? (
                <Image
                  src={`${process.env.NEXT_PUBLIC_PAYLOAD_URL}${props.media.url}`}
                  alt={props.title}
                  w="full"
                ...
                <Link href={`/changelogs/${props.slug}`}>
                  <Image
                    src={`${process.env.NEXT_PUBLIC_PAYLOAD_URL}${props.media.url}`}
                    alt={props.title}
                    w="full"
          ...
          >
            <Box
              ...
              fontSize="lg"
              lineHeight="32px"
              color={useColorModeValue("landing.almostBlack.500", "white")}
            >
              {props.children}
            </Box>
          </motion.div>
